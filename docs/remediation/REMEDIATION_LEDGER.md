...
- **NEW FINDINGS (surfaced post-371):** **372** level-up deity name (FIXED) · **373** combat weapon-triangle armor normalization (→B-13) · **374** consumable buff status-effect shape (→B-09) · **375** floor soul-snapshot param · **376** sound `player.addListener` typing gap · **377** status-effect `statModifier` UNWIRED — Weaken/Slow/Freeze/Curse apply no stat change in combat (P1 modeled-never-wired; confirmed via `grep \.statModifier`) (→B-09). Next audit ID = **378**.
- **Current batch:** **B-09** (one status model, 190) — **SCOPED (investigated 2026-06-14; no code yet — combat-core refactor, not rushed).** Confirmed scope: **two competing `StatusEffect` types** — `types/StatusEffect.ts` (RICH, `type: StatusEffectType`, full helpers) vs `types/Character.ts` (SIMPLE, `id: StatusEffectId`); `Character.statusEffects` persists the simple one, `useCombatStore` uses the rich one's helpers. Plan: collapse to ONE rich model · **wire `statModifier` into combat (377)** · re-represent buffs as a valid effect (374) · migrate the persisted `statusEffects` shape via B-07 deep-merge · sim re-check. **⚠ OWNER DESIGN CALL (review point):** how buffs/debuffs work — (A, rec) unify on the rich model + a generic buff/debuff carrying `statModifier`, wired into the stat calc; (B) model stat-changes separately. Guard: NEW `tests/status.spec.ts` (Weaken lowers effective attack; buff raises the stat; old save hydrates). Then B-10 (favor single-writer) → B-12 (dual-stat finish). **B-08 lint-gate + B-13 (373) ride later.**
