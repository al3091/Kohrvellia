...
- **Current batch:** **B-08** (typed unions + ban `as any`) — **TODO**. **⚠ This is the budgeted error-surfacing slot:** removing `as any` and tightening string-literal unions will EXPOSE latent type errors (the 352 false-green class) — fix-or-ledger each, don't suppress. Also lands the eslint custom rules deferred from B-02 (ban `as any` · balance literals outside `GameConstants` · hex outside `Colors`). Then B-09 (one status model) → B-10 (favor single-writer) → B-12 (dual-stat finish). W1 order per the DAG.
- **DONE:** ✅ **B-01** @ `675e7fd` (**KV-AUD-001 CLOSED**) · ✅ **B-02** @ `5aaa474` (4 CI gates; ratchets armed) · ✅ **B-03** @ `4894974`+`03cdf9d` (**permadeath sealed** — 14 findings; vitest live 11/11) · ✅ **B-04** @ `501017e` + **B-04b** @ `7aabc0f` (**the farm is dead, the exit is open** — 080/081/082/234/246/251/252/254a closed via no-re-arm + banking-completion + floor-cache; traversal is TWO-WAY by design [the exit path — B-04's forward-only was an owner-caught regression, see 13]; moveToNode rejects teleports; 175 WIRED; suite 17/17; sim baseline unchanged) · ✅ **B-05** @ `c0a003e` (**soul single-owner** — 093/109/220/221/232/253 closed; 074 as-designed; 24 dup counters stripped; suite 52) · ✅ **B-06** @ `c27ae21` (**balance source-of-truth reconnected** — 360/151/065/049/068/154; GameConstants now live; golden-master guard; suite 61) · ✅ **B-07** @ `05187bc`+`f809177` (**versioned persistence** — `createVersionedPersist` across ALL 12 stores; 094 behavement reconcile + 118 registry self-heal + 073/088/102/141; suite 72)
