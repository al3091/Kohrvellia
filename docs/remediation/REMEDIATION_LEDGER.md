...
- **Current batch:** B-04 (kill the farm: forward-only map via `getAdjacentNodes`, never un-complete, validate `moveToNode`, persist floors + run-seed, reward-banked⇒complete, retreat-modal→`runFlee`+'fled' state, room BackHandler) — **TODO** · then B-05 (event single-owner) · W0 closes
- **DONE:** ✅ **B-01** @ `675e7fd` (**KV-AUD-001 CLOSED**) · ✅ **B-02** @ `5aaa474` (4 CI gates green; ratchets armed) · ✅ **B-03** @ `4894974`+`03cdf9d` (**permadeath sealed**: 002/069/070/098/104/113/124/138/227/231/248/254b/264/268-deaths closed, 058 deleted; vitest harness live, 11/11; +5 latent timer typings surfaced & fixed — see 13's process correction: gates now run separately from commits)
