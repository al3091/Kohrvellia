...
- **Current batch:** **B-06** (reconnect the balance source-of-truth, #6+G-1) — **TODO**. Scope: adopt the LIVE coefficients INTO `GameConstants` (zero behavior change — the sim must stay byte-identical, that's the guard), make `Stats.ts`/combat/monster code import from it; wire `getProficiencyThreshold` into the growth path behind a constants flag (068/154 — values tuned later in W3); per-field dispositions for the dead 16 configs per registry Cohort 4 (wire-or-defer-to-B-22). Then **B-07** (versioned persistence) ∥ **B-08** (unions + ban-as-any; budget the error-triage slot) → B-09 (one status model) → B-10 (favor single-writer) → B-12 (dual-stat finish). W1 order per the DAG.
- **DONE:** ✅ **B-01** @ `675e7fd` (**KV-AUD-001 CLOSED**) · ✅ **B-02** @ `5aaa474` (4 CI gates; ratchets armed) · ✅ **B-03** @ `4894974`+`03cdf9d` (**permadeath sealed** — 14 findings; vitest live 11/11) · ✅ **B-04** @ `501017e` + **B-04b** @ `7aabc0f` (**the farm is dead, the exit is open** — 080/081/082/234/246/251/252/254a closed via no-re-arm + banking-completion + floor-cache; traversal is TWO-WAY by design [the exit path — B-04's forward-only was an owner-caught regression, see 13]; moveToNode rejects teleports; 175 WIRED; suite 17/17; sim baseline unchanged)
