...
- **Phase:** ★ **EXECUTION — WAVE 2** *(W0 + W1 COMPLETE; GATE G1 SIGNED by the owner 2026-06-10)*
...
- **D8 (post-G1 design directive, 2026-06-14):** "The Tower Fights Back" — `huntPressure` anti-farm + a shared `revelationStage` clock + staged deity level-up lines. Monsters repopulate on floor re-entry (loot/items/stat/soul KEPT; only monsters repopulate — reward nodes stay once); pressure cools ONLY by descending to NEW depth; Hunter = a dispatched *venoium*; severity A→B→C. **Canon refined:** the patron marks you BLIND by luck, the player is unaware, the reveal is drip-fed (no lore term before it's earned). **Full spec → `docs/DESIGN_HUNT_AND_REVELATION.md`**; canon note in `docs/LORE_GAME_INTEGRATION.md`. Build = **W4/B-34** (needs B-07 + B-15). **DECIDED 2026-06-14: A→B→C tiered severity + cross-dive "ledger remembers" seal ON** (per-character `huntMemory`, resets on death). Only the diegetic naming remains (lore pass).
- **★★ WAVE 0 COMPLETE (B-01…B-05 + B-04b).** Build tracked+gated · permadeath sealed · farm dead, exit open · soul ledger honest. 3 vitest suites / 52 tests + 4 CI gates stand guard.
- **B-06 DONE** (`c27ae21`): GameConstants reconnected — `Stats.ts`/`Monster.ts` now read the LIVE coefficients (the ~10× drift fixed IN the constants, not the code); proficiency threshold wired (068/154); monster scaling relocated to GameConstants (B-15's single home). Guard: golden-master `derived-stats-characterization.spec` + sim byte-identical. Dead-16 disposition logged in `13` (3 wired, rest deferred to B-22). **vitest now 4 files / 61.**
- **B-07 CORE DONE** (`05187bc`): NEW `src/lib/createVersionedPersist.ts` (version + migrate + **deep-merge of saved over defaults** — new nested fields never hydrate undefined; dynamic maps keep entries). **094 CLOSED** (`reconcileBehavements` rebuilds the soul array vs current definitions on load) · **118 CLOSED** (`weaponRegistry` self-heal + `rebuildWeaponRegistry`). Adopted in **useSoulStore + useDungeonStore** (latter retires the `-v2` key-bump; D8 fields will hydrate cleanly). Guard `tests/migrate.spec.ts` (11). **vitest now 5 files / 72.** Two bugs caught by the pre-commit sweep (logged in `13`).
- **B-07 DONE** (`f809177`): rolled `createVersionedPersist` into the remaining 10 stores (game · deity · market · character · achievements · job · blacksmith · inventory-ui · sacred · shop) — **all 12 persisted stores now versioned; 073/088/102/141 CLOSED.** Sweep green (tsc 0 · vitest 72 · refint · knip · madge 23). Residual → **B-27**: weaponRegistry consumer-side rebuild-on-miss (the registry already self-heals corruption). B-09 may now safely change a store's shape (all stores carry the deep-merge).
- **B-08 SURFACING PASS DONE** (`cd4f3b2`): the `as any` reality — most were tests/audit (legit) or the WORD "as any" in flavor text (false positives); ~10 real game-code casts; **NO eslint configured.** **Fixed 372** (real bug: level-up showed the deity name via a placeholder `(character as any).deity` that read a non-existent field → always "Your Patron Deity"; now reads `useDeityStore`) + 2 clean casts. **Surfaced + ledgered 373–376** (combat weapon-triangle armor normalization · consumable buff status-effect shape · floor soul-snapshot param · sound `addListener` library gap) → fixed in their domain batches. Sweep green (tsc 0 · 72 · refint · knip · madge 23). **Deferred (documented):** the ban-as-any / balance-literal / hex **regression gate needs eslint stood up** → dedicated tooling task (recommend a grep-style gate matching this project's pattern); 3 minor equipment casts = cleanup tail.
- **NEW FINDINGS (surfaced post-371):** **372** level-up deity name (FIXED) · **373** combat weapon-triangle armor normalization (→B-13) · **374** consumable buff status-effect shape (→B-09) · **375** floor soul-snapshot param · **376** sound `player.addListener` typing gap · **377** (CORRECTED) TWO DISCONNECTED status systems: combat (`useCombatStore`, rich `StatusEffect.ts`, effects work via HARDCODED logic on its own ephemeral list) vs persisted `character.statusEffects` (simple `Character.ts`, divergent enum) — **combat never reads the character list, so consumable buffs + persistent afflictions never reach combat** (`statModifier` field decorative in both). [supersedes the earlier "statModifier unwired" framing] (→B-09). Next audit ID = **378**.
- **B-09 DONE (core)** — ONE status model achieved (1a `f49df2c` vocab union · 1b `697f475` type collapse · 3 `30c8bc1` the BRIDGE · 4 `2f615ad` attack statModifier · 2 `ba2bfdc` buff potions). **190 + 374 CLOSED.** Buffs + debuffs now actually work in combat: weaken/curse cut attack, curse blocks healing, paralysis/stun/freeze skip turns, poison/bleed/burn DoT, Strength buffs raise attack. Character persist → v2 (migrate clears old-shape effects). Guards `status.spec` + `status-bridge.spec`; **suite 83**; sim unchanged throughout. **Follow-up (step 4b, small, NON-blocking):** non-attack `statModifier`s (AGI→speed: Slow/Freeze/Agility) in the speed calc + the out-of-combat buff-use path.
- **B-10 CORE DONE** (`68e9d31`): `adjustFavor` is the SOLE favor writer — mirrors onto `character.deityFavor` (new absolute `setDeityFavor`, keeps the `computeMaxResources` blessing recompute) so favor can't diverge; eviction CLEARS on recovery; room.tsx double-write removed. **121/242/270 closed.** Guard `favor.spec`; suite **86**. **Step 2 (tail):** route the 4 internal challenge favor-sets (complete/fail/expiry) through `adjustFavor`; FAVOR_TIERS collapse; challenge-reward targeting (closes the rest: 180/122/123/079/215/126).
- **★ B-10 COMPLETE** (`68e9d31`+`736c1ba`+`faa479f`): `adjustFavor` is the SOLE favor writer (mirrors `character.deityFavor` + recomputes blessing maxHP/maxSP) · eviction clears on recovery · all live favor paths routed (shrine/prayer/domain/challenge) · challenge rewards target the build's top stats. **121/242/270/180/122 CLOSED.** Guard `favor.spec`; suite **86**. **Residue → B-22 (cosmetic):** FAVOR_TIERS collapse + dead `tickChallengeFloor`.
- **★★ B-12 DONE** (`5e02d61`): dual-stat weapons engage their 2nd physical stat — `physScaling` splits evenly across a weapon's physical `primaryStats` (sum 1.0, no double-dip); single-stat byte-identical (golden master + sim). **312 CLOSED.** Guard `weapon-split.spec`; suite **89**. Residual: 071 dup-extraction (→B-22); owner-review even-vs-weighted split.
- **★★★ WAVE 1 COMPLETE** (B-06·B-07·B-08·B-09·B-10·B-12). Riders/residue tracked: B-08 lint-gate · B-09 step 4b (speed) · B-10 FAVOR_TIERS · B-12 071.
- **Current batch:** **WAVE 2 — B-11 THE EFFECT-RESOLVER** (#4, the audit's keystone) — **TODO** *(scope read on entry)*. One `resolveEffect(effect, ctx)` registry shared by combat/skills/consumables/events; registers statModifier/healing/suffix-procs/buff/damage_percent/flee/penalties/afflictions/achievement-checkers. Dep B-08·B-09 (done). Closes 052·057·116·191·192·193·194·199·224·301·336·341·327·333. Guard `tests/resolver.spec.ts` (one case per type, failing-first) + dpr5_check. **L (the seam) + XS per registration.** → then B-13/B-14.
- **DONE:** ✅ **B-01** @ `675e7fd` (**KV-AUD-001 CLOSED**) · ✅ **B-02** @ `5aaa474` (4 CI gates; ratchets armed) · ✅ **B-03** @ `4894974`+`03cdf9d` (**permadeath sealed** — 14 findings; vitest live 11/11) · ✅ **B-04** @ `501017e` + **B-04b** @ `7aabc0f` (**the farm is dead, the exit is open** — 080/081/082/234/246/251/252/254a closed via no-re-arm + banking-completion + floor-cache; traversal is TWO-WAY by design [the exit path — B-04's forward-only was an owner-caught regression, see 13]; moveToNode rejects teleports; 175 WIRED; suite 17/17; sim baseline unchanged) · ✅ **B-05** @ `c0a003e` (**soul single-owner** — 093/109/220/221/232/253 closed; 074 as-designed; 24 dup counters stripped; suite 52) · ✅ **B-06** @ `c27ae21` (**balance source-of-truth reconnected** — 360/151/065/049/068/154; GameConstants now live; golden-master guard; suite 61) · ✅ **B-07** @ `05187bc`+`f809177` (**versioned persistence** — `createVersionedPersist` across ALL 12 stores; 094 behavement reconcile + 118 registry self-heal + 073/088/102/141; suite 72) · ✅ **B-08** @ `cd4f3b2` (**error-surfacing pass** — fixed 372 [level-up deity name] + 2 casts; surfaced 373–376; eslint reality documented)
