...
- **Current batch:** B-05 (event-firing single-owner, P15/G-4) — **TODO**. Scope: strip the ~30 screen-side duplicate behavement calls from `combat.tsx` (the store owns gameplay events — the 093/220 double-count); fix the magic-kills→`phys_consecutive_kills` copy-paste (221, `combat.tsx:457,500` — use a magic streak or drop); evaluate the 4 per-floor-conduct behavements on real descents in `floor.tsx handleDescend` (232/253: `magic_no_physical`/`risk_no_heal_floor`/`caution_no_traps`/`glory_perfect_floor`); add the INT/LCK growth cases to `useCharacterStore.commitExcelia`'s soul switch (074, `:444-449`); fire `resource_sell_items`+`incrementShopSpend` from `useShopStore.sellItem` (109). Guard: NEW `tests/soul-singlefire.spec.ts` (one attack ⇒ each behavement exactly +1; per-floor conducts fire on a non-boss descent). W0 closes with this batch.
