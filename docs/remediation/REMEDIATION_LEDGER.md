...
- **Phase:** B — RECONCILE & TRIAGE *(Phase A ★ COMPLETE — all 371 rows in the register)*
- **Current unit:** B1 (machine-regenerated severity tally from CANON headers + status-roll-up of the register → `09_TRIAGE_REPORT.md` skeleton) — **TODO**
- **Then:** B2 (HEAD re-verification: S0 ✅done + the 4 remaining S1 recipes + the NEEDS-REPRO list below) → B3 (buckets A–F + F protect-list)
- **Phase-B NEEDS-REPRO worklist (from OBS-11 + A9):** 169 MonsterAbility · 192 getHealingModifier · 177-tail (4 passives) · 239 skill-scale · 249 mystery-reveal source · 034 EnemyPreview · 037-tail (Easing/Breakpoint/Shadow.glow) · 032-roster delta (GritPanel/AnimatedHP/SPBar vs knip) · FloatingDamage x-offset (03 §2) · 341 event-status sample · 181 domain-blessing sample · 363 junk tracked-vs-untracked delta · 342-nit event statCheck.dc raw-points
...
| 032 | S2 | PART-CONF | S3 | Agent | P7 | components | EmphasisText; StatTooltip; ActionModal; GritPanel; AnimatedHP/SPBar | →354/358 (partial roster) | — | — | #15 | ⚠ A9: GritPanel + AnimatedBars NOT knip-flagged; 369 shows AnimatedBar USED |
| 033 | S2 | SUPERSEDED | S3 | Conf | — | components/perf | `src/components/**` | →356 (+369) | — | — | #18 | 0/49 React.memo; JS-thread HP/SP bars |
| 034 | S2 | NEEDS-REPRO | S2? | Agent | — | components | `EnemyPreview.tsx:58,64` | 03 §2 no-op listing; not discharged by 356–371 | — | — | — | ⚠ A9/Phase-B: hardcoded level-1 + dead `canInflict` |
...
| 037 | S3 | PART-CONF | S3 | Agent | P7 | constants | `Animation.ts:112-125,224`; `Spacing.ts:115` | refutes AUD-079 | — | — | #15 | ⚠ A9: dead Easing/Breakpoint not named by knip |
...
| 169 | S3 | NEEDS-REPRO | S3? | Likely | — | monster | `Monster.ts:114-124` | "verify Wave 2" never discharged (OBS-11) | — | Phase-B grep | — | `MonsterAbility` likely dead (enemyTurn uses behaviorPattern only). ⚠ §4 mis-maps this ID to biomes |
...
| 177 | S2 | REFINED | S2 | Conf | P1 | paragon/passives | `Behavement:158-217` vs `useCharacterStore:1350-82` | part-refuted 223 (gold_bonus screen-side); 4 others OPEN (OBS-11) | — | Phase-B sample | #4 | "inert" → "applied inconsistently, never in derived path" |
...
| 192 | S2 | NEEDS-REPRO | S2? | Likely | P1 | combat/healing | `StatusEffect.ts:152,73,244-257` | OPEN at audit close (OBS-11) | — | Phase-B grep | — | is `getHealingModifier` ever called? curse/burn healing-mods at stake |
...
| 239 | S3 | NEEDS-REPRO | S3? | Conf(input)/open(scale) | — | skills/scaling | `combat.tsx:722,1778` | "verify in W3-DPr" never discharged (→OBS-11) | — | Phase-B | — | skills staged with raw 0-999 points; is calculateSkillDamage effStat-scaled? |
...
| 249 | S3 | NEEDS-REPRO | S3? | Conf | — | dungeon/mystery | `room.tsx:789-798,1172-1205,1478-1488` | "needs W3 gen cross-ref" never discharged (→OBS-11) | — | Phase-B | — | reveal handles combat/elite/rest the generator may never emit |
...
| 341 | S3 | LIKELY | S3 | Likely | P1/P10 | events/ramifications | `dungeonEvents.ts` + `ramifications.ts:32-41` | ×190/268/057 | — | Phase-B sample | #4 | resource drains work; status-afflictions likely display-only |
...
| 363 | S3 | CONFIRMED | S3 | Conf | — | repo hygiene | repo root (17 files) | DISCHARGES seed 012 | — | Phase-B: tracked-vs-untracked delta (seed said untracked; 363 says tracked) | #19 | botched-redirect garbage; config files all present + conventional |
