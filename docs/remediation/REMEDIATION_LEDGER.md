...
- **Current batch:** **B-10** (favor single-writer) — **TODO**. Goal: one authoritative writer for deity favor — consolidate the scattered `favor`/`adjustFavor`/`modifyDeityFavor` mutations so favor changes are consistent, clamped, and traceable through a single path. Then **B-12** (dual-stat finish). **Riding later: B-08 lint-gate · B-13 (373 combat triangle) · B-09 step 4b.**
