...
- **Current batch:** **B-09** (one status model, 190) — **FULLY SCOPED; OWNER PICKED OPTION A. Execution = next (combat-core, careful multi-step; not rushed at marathon-tail).** Architecture (mapped 2026-06-14): combat (`useCombatStore`, RICH `StatusEffect.ts`, hardcoded per-effect logic, own ephemeral list reset per encounter) is DISCONNECTED from persisted `character.statusEffects` (SIMPLE `Character.ts`, divergent enum, written by consumables/room-events/ramifications/monsters) — combat never reads it, so buffs/afflictions don't reach combat. **Ordered plan (Option A):** (1) one canonical model in `StatusEffect.ts` + reconcile enum to the UNION (add fear/silence/paralysis); re-point `Character.ts`/`Monster.ts`/`addStatusEffect`. (2) generic buff/debuff effect. (3) **bridge** combat `playerEffects` ↔ `character.statusEffects` at encounter start/end. (4) apply the effect's stat delta in the combat calc (wire `statModifier` / consolidate the hardcoded logic) — Thane %-vs-flat call. (5) migrate persisted shape via B-07 deep-merge. (6) guard `tests/status.spec.ts` (buff actually raises a stat in combat; weaken lowers damage; old save hydrates) + sim re-check. Then B-10 (favor single-writer) → B-12 (dual-stat finish). **B-08 lint-gate + B-13 (373) ride later.**
