...
- **Current batch:** **B-07** (versioned persistence, P13/G-6) — **TODO**. **⚠ MUST land before any persisted-shape change (B-09).** Scope: one `createVersionedPersist` helper (version + migrate + settings-merge) for the Zustand persist stores; behavement-array merge on load (094); persist-or-self-heal the `weaponRegistry` everywhere (118). **Closes 073 · 088 · 094 · 102 · 141 · 118.** Guard: NEW `tests/migrate.spec.ts` — an old-shape save fixture hydrates clean (no crash, no data loss). Effort **S**. Then **B-08** (unions + ban-as-any; budget the error-triage slot) → B-09 (one status model) → B-10 (favor single-writer) → B-12 (dual-stat finish). W1 order per the DAG.
- **DONE:** ✅ **B-01** @ `675e7fd` (**KV-AUD-001 CLOSED**) · ✅ **B-02** @ `5aaa474` (4 CI gates; ratchets armed) · ✅ **B-03** @ `4894974`+`03cdf9d` (**permadeath sealed** — 14 findings; vitest live 11/11) · ✅ **B-04** @ `501017e` + **B-04b** @ `7aabc0f` (**the farm is dead, the exit is open** — 080/081/082/234/246/251/252/254a closed via no-re-arm + banking-completion + floor-cache; traversal is TWO-WAY by design [the exit path — B-04's forward-only was an owner-caught regression, see 13]; moveToNode rejects teleports; 175 WIRED; suite 17/17; sim baseline unchanged) · ✅ **B-05** @ `c0a003e` (**soul single-owner** — 093/109/220/221/232/253 closed; 074 as-designed; 24 dup counters stripped; suite 52) · ✅ **B-06** @ `c27ae21` (**balance source-of-truth reconnected** — 360/151/065/049/068/154; GameConstants now live; golden-master guard; suite 61)
