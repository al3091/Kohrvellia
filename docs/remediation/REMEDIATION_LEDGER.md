...
- **NEW FINDINGS (surfaced post-371):** **372** level-up deity name (FIXED) · **373** combat weapon-triangle armor normalization (→B-13) · **374** consumable buff status-effect shape (→B-09) · **375** floor soul-snapshot param · **376** sound `player.addListener` typing gap. Next audit ID = **377**.
- **Current batch:** **B-09** (one status model, #?) — **TODO**. Goal: a single source of truth for status effects (consolidate the duplicate/competing status-effect shapes); **absorbs 374** (the buff built via `as unknown as addStatusEffect` param). ⚠ It changes a persisted shape → relies on B-07's deep-merge (now in place). Then B-10 (favor single-writer) → B-12 (dual-stat finish). W1 order per the DAG. **B-08 lint-gate + B-13 (373) ride later.**
