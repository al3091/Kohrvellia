...
| BUG-023 | `app/dungeon/boss-cleared.tsx` | Screen exists and is complete but is currently unreachable — no routing points to it. With per-run boss fights, the screen only makes sense as an intro to the fight (showing that guild history exists) but it's not integrated. | 2026-05-15 | RESOLVED — 2026-05-18 (full): floor.tsx now checks `gameState.defeatedBosses.includes(milestoneBoss.id)` before routing to boss-encounter. If previously defeated, routes to boss-cleared (lore tablet) first. Layout already registered. |
...
| BUG-025 | `src/data/events/dungeonEvents.ts` | `runFlags` system for multi-step events has no documentation of flag names, semantics, or valid values. Flag names like `'gamblers_coin'` are string literals scattered across the codebase with no registry. | 2026-05-15 | RESOLVED — 2026-05-18: `DUNGEON_FLAGS` const registry added to `src/constants/GameConstants.ts` documenting all valid flag strings and their semantics. |
...
| BUG-009 | `app/index.tsx` | `epitaph` text on title screen is computed via `useMemo` with empty dependency array (stable on mount). The `// eslint-disable-line` comment suppresses a legitimate warning. If `lastDeath` changes mid-session (player starts a new run and dies quickly), the epitaph is stale. Low impact but worth noting. | 2026-05-06 | RESOLVED — 2026-05-18: Added `lastDeath` to the memo dependency array. Removed eslint-disable comment. |
...
| BUG-012 | `useCharacterStore.performLevelUp()` | After `performLevelUp()`, the function calls `unlockAchievementsForLevel(updatedCharacter.level)`. After incrementing to level 2, this revealed `targetLevel: 2` achievements (L1→L2) — the transition the player just completed. Should reveal `targetLevel: 3` (L2→L3) so the player knows what to work toward next. | 2026-05-06 | RESOLVED — 2026-05-18: Changed to `unlockAchievementsForLevel(updatedCharacter.level + 1)` to reveal the NEXT transition's standard achievements. |
| BUG-013 | `useDungeonStore` | No store reset is called when entering the dungeon from a fresh character (after character creation → tutorial → town → dungeon). If any dungeon state persists from a previous session that wasn't properly cleared, floor generation may use stale data. | 2026-05-06 | RESOLVED — `clearAllStores()` calls `useDungeonStore.getState().clearAllData()` which sets `currentRun: null`. Town hub `enterDungeon()` only calls `startNewRun()` when `!currentRun`. Flow is correct. |
